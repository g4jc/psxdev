...
static char rcsid[] ="$Id: write.c,v 1.1.1.1 2001/05/17 11:50:28 dbalster Exp $";
...
void FDECL4(xfwrite, void *, buffer, int, count, int, size, FILE *, file)
{
	...
	/* psx/XA hack: negative size is used to indicated XA files */
	if (size<0)
	{
	...
	{
		char *dummy = alloca (2336-2048);
		int n, i;
		
		n = size/2048;
		memset (dummy,0,2336-2048);
		
		for (i=0; i<n; i++)
		{
			fwrite (dummy,8,1,file);				// empty XA header
			fwrite ((buffer+(i*2048)),2048,1,file);
			fwrite (dummy, 2336-2048-8, 1, file);	// empty XA footer
		}

/*		char *buf = alloca (2352);
		int n, i;
		...
		for (i=0; i<n; i++)
		{
			memcpy (buf+16,(buffer+(i*2048)),2048);
	//		do_encode_L2 (buf,MODE_1,0);
			fwrite (buf+8, 2336, 1, file);	// empty XA footer
		}
*/
	}
}
...
{
     char		  buffer[XA_SECTOR_SIZE * NSECT];
     FILE		* infile;
     int		  remain;
     int		  use;
	 int		psx_xa = 0;

	extern int xafiles_find (char *filename);

	psx_xa = xafiles_find(filename);
     ...
     while(remain > 0)
     {
	  ...
	  if (psx_xa == 1)
	  {
	  	int xasize = (use/2048)*2336;
	  
		  if (fread(buffer, 1, xasize, infile) == 0) 
		  {
			fprintf(stderr,"cannot read from %s\n",filename); 
			exit(1);
		  }
		  xfwrite(buffer, 1, -xasize, outfile);
	  }
	  else
	  {
...
static void FDECL1(write_files, FILE *, outfile)
{
     ...
     while(dwpnt)
     {
	  if(dwpnt->table) 
	  {
	  fprintf(stderr,"writing files table\n");
...
fprintf(stderr,"writing directory tables\n");
	...
	fprintf(stderr,"writing continuation entry record\n");
...
static int FDECL1(pvd_write, FILE *, outfile)
{
  ...
  memset(vol_desc.application_data,0,sizeof(vol_desc.application_data));
  memcpy(vol_desc.application_data+141,"CD-XA001",8);

fprintf(stderr,"writing volume descriptor (1)\n");
...
static int FDECL1(evd_write, FILE *, outfile)
{
  ...
  memcpy(vol_desc.id, ISO_STANDARD_ID, sizeof(ISO_STANDARD_ID));
  vol_desc.version[0] = 1;
fprintf(stderr,"writing volume descriptor (2) = \"%s\"\n",ISO_STANDARD_ID);
  xfwrite(&vol_desc, 1, 2048, outfile);
...
static int FDECL1(pathtab_write, FILE *, outfile)
{
   ...
   */
fprintf(stderr,"writing path_table_l & path_table_m\n");
...
{
fprintf(stderr,"writing extension_record\n");
...
fprintf(stderr,"writing \"%s\" into system area\n",license);

{
	...
	{
		p = malloc (12 * 2336);
		if (p == NULL) { fprintf(stderr,"memory problem\n"); exit(1); }
		i = fread (p,12*2336,1,f);
		if (i != 1) { fprintf(stderr,"fread failed\n"); exit(1); }
    	fwrite(p, (12*2336), 1, outfile);
		free (p);
	}
	else
	{
		fprintf(stderr,"Warning: couldn't open license - no license is written to the system area\n");
		pad = 12;
	}
}

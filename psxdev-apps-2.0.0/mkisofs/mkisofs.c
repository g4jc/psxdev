...
static char rcsid[] ="$Id: mkisofs.c,v 1.1.1.1 2001/05/17 11:50:28 dbalster Exp $";
...
//dbalster-begin
#define OPTION_XAFILES		3000
#define OPTION_LICENSE		3001

char *license = "/opt/psxdev/share/license";

// read in the XA file list

#define MAXXAMATCH 1000
static char *xamat[MAXXAMATCH] = {0};

#include <fnmatch.h>

void xafiles_add (char *fn)
{
  register int i;

  for (i=0; xamat[i] && i<MAXXAMATCH; i++);
  if (i == MAXXAMATCH) {
    fprintf(stderr,"Can't exclude RE '%s' - too many entries in table\n",fn);
    return;
  }
  xamat[i] = (char *) malloc(strlen(fn)+1);
  if (! xamat[i]) {
...
// silly search for a filename in the list

int xafiles_find (char *filename)
{
  /* very dumb search method ... */
  register int i,x;
  for (i=0; xamat[i] && i<MAXXAMATCH; i++) {
    if (fnmatch(xamat[i], filename, 0)==0) {
...
{
  { {"xa-files", required_argument, NULL, OPTION_XAFILES},
      'X', "GLOBFILE", "pattern which matches XA files (PSXDEV)" , ONE_DASH },
  { {"psx-license", required_argument, NULL, OPTION_LICENSE},
      'I', "FILE", "license file for system area (PSXDEV)" , ONE_DASH },

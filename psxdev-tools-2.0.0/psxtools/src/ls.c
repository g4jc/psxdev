/* $Id: ls.c,v 1.1.1.1 2001/05/17 11:50:26 dbalster Exp $ */
...
static struct option long_options[] =
{
	...
	{ "long" ,no_argument, NULL, OPTION_LONG },
	{ "comment" ,no_argument, NULL, OPTION_COMMENT },
	{ NULL }
};
...
static char *option_help[] = {
	...
	N_("list in long format"),
	N_("show sjis encoded file comment"),
};
...

	{
		...
		if (pccl != -1)
		{
			...
			for (i=0; i<dir.count; i++)
			{
				if (listlong)
				{
					printf1 ("%2d: %6d \"%-20s\"",i,(int)dir.file[i].size,dir.file[i].name);
				}
				else
				{
					printf1 ("%-20s",dir.file[i].name);
				}

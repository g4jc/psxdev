/* $Id: mkfs.c,v 1.1.1.1 2001/05/17 11:50:26 dbalster Exp $ */
...
warnf (_("THIS APPLICATION DOES NOT YET WORK CORRECTLY - USE AT YOUR OWN RISK"));
...

	{
		...
		if (pccl != -1)
		{
			...
			err = ioctl(pccl,CAEIOC_MC_FORMAT,&arg);
			if (err < 0) ioerrorf("ioctl(CAEIOC_MC_FORMAT)");

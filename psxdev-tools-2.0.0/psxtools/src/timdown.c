/* $Id: timdown.c,v 1.1.1.1 2001/05/17 11:50:26 dbalster Exp $ */
...
static char *option_help[] = {
	...
	N_("left edge of image area (default: caetla FBV)"),
	N_("top edge of image area (default: caetla FBV)"),
	N_("width of image area (default: caetla FBV)"),
	N_("height of image area (default: caetla FBV)"),
...
int main (int argc, char **argv)
{
	...
	*/
	...
	if (pccl != -1)
	{
		...
		rc = ioctl (pccl,CAEIOC_FB_GET_MODE,&fbm);
		if (rc == -1) iowarnf("ioctl(CAEIOC_FB_GET_MODE)");
		...
		printf2 (_("Downloading +%d+%d %dx%d (mode %d)\n"),
		fbm.x, fbm.y, fbm.width, fbm.height, fbm.mode);

		size = fbm.width * fbm.height * 2;
		mode = (fbm.mode==0) ? TIM_15BIT : TIM_24BIT;
		
		tim = tim_new (mode,fbm.width,fbm.height,0);
		if (tim != NULL)
		{
			void *pixel = tim_pixel (tim,0,0);
		
			rc = ioctl (pccl,CAEIOC_FB_READ_MEMORY,&fbm);
			if (rc == -1) ioerrorf("ioctl(CAEIOC_FB_READ_MEMORY)");

			rc = read (pccl,pixel,size);
			if (rc < 0) ioerrorf("read()");

			if (1 != fwrite (tim,tim_sizeof(tim),1,file)) iowarnf ("fwrite()");

...
void reinitXaDecode(char i)
{
	at1[i] = at2[i] = at1_x[i] = at2_x[i] = 0;
}
void switchXaDecode(char i)
{
	t1 = at1[i];
	t2 = at2[i];
	t1_x = at1_x[i];
...
{
	return(ss->sectorFiller[XAMSF]<<24|ss->sectorFiller[XAMSF+1]<<16|ss->sectorFiller[XAMSF+2]<<8|ss->sectorFiller[XAMSF+3]);
}
